...
#notes/things to fix/change/or add:
    #
#GUI
    #First iteration should focus only on functionaility worry about looks later we'll use tkinter

#Maybe add:
    # I also want to store test scores so user can see they are learning this may be pushed to a later iteration due to time
...
class Deck:
    ...
    def add_flashcard(self):
        while True:
            question = input("Enter the question or enter quit to quit: ")
...
class FlashcardApp:
    ...
    def test_mode(self):
        ...
        while len(used_flashcards) < len(deck.flashcards):
            for flashcard in flashcards:
                ...
                used_flashcards.add(flashcard)
        print("You have completed all of the questions for this deck!")
    ...
    def deck_manager_menu(self):
        while True:
            ...
            if choice == "1" or choice == "Create Deck":
                self.create_deck()
            elif choice == "2" or choice == "View Decks":
                self.view_decks()
            elif choice == "3" or choice == "Add Flashcard to a Deck":
                deck = self.select_deck()
                if deck:
                    deck.add_flashcard()
                    self.save_decks()
            elif choice == "4" or choice == "View FLashcards in a Deck":
                deck = self.select_deck()
                if deck:
                    deck.view_flashcards()
            elif choice == "5" or choice == "Remove Flashcard from a Deck":
                deck = self.select_deck()
                if deck:
                    deck.remove_flashcard()
                    self.save_decks()
            elif choice == "6" or choice == "Remove a Deck":
                self.remove_deck()
            elif choice == "7" or choice == "Back to Main Menu":
                break
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == "1" or choice == "Deck Manager":
                self.deck_manager_menu()
            elif choice == "2" or choice == "Practice Mode":
                self.practice_mode()
            elif choice == "3" or choice == "Test Mode":
                self.test_mode()
